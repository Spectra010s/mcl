...
export async function generateMetadata(props: PageProps): Promise<Metadata> {
  ...
  const title = isGeneralCourse
    ? `${level?.level_number} Lvl ${dept?.full_name}`
    : `${level?.level_number} Lvl ${dept?.full_name} Department Courses`
...
export default async function CoursesPage(props: PageProps) {
  ...
  return (
    <>
      <script
        id={`breadcrumb-lvl-${levelId}`}
        key={`breadcrumb-${facultyId}-${departmentId}-${levelId}`}
